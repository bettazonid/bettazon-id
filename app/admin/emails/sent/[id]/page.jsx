...
export default function AdminSentEmailDetailPage() {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await adminFetch(`/emails/admin/sent/${params.id}`)
      setEmail(response.data || null)
    } catch (err) {
      setError(err.message || 'Gagal memuat detail sent email')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return <div className="text-gray-600">Memuat detail sent email...</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Detail Sent / Outbox</h1>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="font-medium text-gray-900 break-all">{email.messageId || '-'}</p>
          </div>
        </div>

        <div>
          <p className="text-sm text-gray-600">Waktu Kirim</p>
          <p className="font-medium text-gray-900">{email.sentAt ? new Date(email.sentAt).toLocaleString('id-ID') : '-'}</p>
        </div>

        {!!email.errorMessage && (
          <div>
            <p className="text-sm text-red-600">Error</p>
            <p className="font-medium text-red-700">{email.errorMessage}</p>
          </div>
        )}
